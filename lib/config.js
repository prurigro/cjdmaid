...
config.CJDMAID_CONFIG_PATH = getUserHome() + "/.config/cjdmaid.conf";
config.CJDMAID_CONFIG_NAME = "cjdmaidConf";

config.CONFIGS_COMMENTS = {
    "cjdmaidConf": "\n\t\tcjdmaid config: ~/.config/cjdmaid.conf\n\n\t\t * edit these values to match your system\n\t\t * passwordLength sets the length of new passwords\n\t\t * The settings in ownNodeData are optional\n\t"
};

config.readJson = function(jsonfile) {
    return when(
    nodefn.call(fs.readFile, jsonfile, "utf8")).then(function(data) {
...
config.readCjdmaidConf = function() {
    return when(
    config.readJson(config.CJDMAID_CONFIG_PATH)).then(function(cjdmaidJson) {
        var err;
        ...
        cjdmaidJson.cjdrouteConf = util.expanduser(cjdmaidJson.cjdrouteConf);
        ...
        cjdmaidJson.cjdnsadminConf = util.expanduser(cjdmaidJson.cjdnsadminConf);
...
config.readCustomConf = function(confname) {
    ...
    return when(
    config.readCjdmaidConf()).then(function(cjdmaidJson) {
...
config.writeJson = function(jsonfile, document) {
    ...
    return when(
    nodefn.call(fs.writeFile, jsonfile, writingText)).otherwise(function(err) {
        if (err) {
            return util.panic(err);
        }
    });
...
config.writeCustomConfSafe = function(confname, document) {
    if (confname === config.CJDMAID_CONFIG_NAME) {
        return when(
        nodefn.call(fs.readFile, config.CJDMAID_CONFIG_PATH, "utf8")).then(function(data) {
            var backup = util.expanduser("~/." + path.basedir(config.CJDMAID_CONFIG_PATH) + "-back");
    ...
    return when(
    config.readCjdmaidConf()).then(function(readed) {
        cjdmaidJson = readed;
        return nodefn.call(fs.readFile, cjdmaidJson[confname], "utf8");
    }).then(function(data) {
        var backup = util.expanduser("~/." + path.basename(cjdmaidJson[confname]) + "-back");
